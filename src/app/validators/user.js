const User = require("../models/user")
const {compare} = require("bcryptjs")

function checkAllFields(body) {
    const keys = Object.keys(body)
    for (key of keys) {
        if (body[key] == "") {
            return {user: body, error: "Preencha todos os campos!"}
        }
    }
}

async function show(req, res, next) {
    const {userId: id} = req.session
    const user = await User.findOne({where: {id}})
    if (!user) return res.render("/", {error: "Usuário não encontrado."})
    req.user = user
    next()
}
async function post(req, res, next) {
    const fillAllFields = checkAllFields(req.body)
    if (fillAllFields) return res.render("user/register", fillAllFields)
    let {email, cpf_cnpj, password, passwordConfirm} = req.body
    cpf_cnpj = cpf_cnpj.replace(/\D/g, "")
    const user = await User.findOne({where: {email}, or: {cpf_cnpj}})
    if (user) return res.render("user/register", {user: req.body, error: "Usuário já cadastrado."})
    if (password != passwordConfirm) return res.render("user/register", {user: req.body, error: "Senhas não conferem."})
    next()
}
async function update(req, res, next) {
    const fillAllFields = checkAllFields(req.body)
    if (fillAllFields) return res.render("user/index", fillAllFields)
    const {id, password} = req.body
    if (!password) return res.render("user/index", {user: req.body, error: "Digite sua senha para prosseguir."})
    const user = await User.findOne({where: {id}})
    const passwordCheck = await compare(password, user.password)
    if (!passwordCheck) return res.render("user/index", {user: req.body, error: "Senha incorreta."})
    req.user = user
    next()
}

module.exports = {post, show, update}